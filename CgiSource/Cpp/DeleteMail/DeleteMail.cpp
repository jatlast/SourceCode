...
//	cout << HTTPHTMLHeader() << endl << flush;
	...
	string Program_Name("DeleteMail.exe");
	string caller = "";
...
//		cout << HTTPHTMLHeader() << endl << flush;
	...
	DB_Table_Base * Base_Ptr = new Login_Info("deleteMail");
	
	
	cfl.Html_Begin(Map, 1, DELETE_MAIL);
	/*
	...
	*/
	...
	form_iterator caller_itr = cgi.getElement("caller");
	...
	if( mail_id != cgi.getElements().end() && cookies_found )
	{
		...
		else if(Return_Status == true)
		{
			...
			if(caller == "GenInbox.exe")
				cfl.Html_AddRedirect(Map.PrintValue( "CGIBIN" ), "/GenInbox.exe");	
			else if(caller == "GenOutbox.exe")
				cfl.Html_AddRedirect(Map.PrintValue( "CGIBIN" ), "/GenOutbox.exe");	
		}
		else
		{
