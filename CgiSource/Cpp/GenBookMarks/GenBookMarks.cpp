/************************************************************************
*                                                                       *
*  PROGRAM ID         : GenBookMarks.cpp							    *
*                                                                       *
*  DESCRIPTION        : Displays the user's saved bookmarks.            *
*																		*
...
//		cout << HTTPHTMLHeader() << endl << flush;
	...
	DB_Table_Base * Base_Ptr = new Login_Info("genBookMarks");

	//cfl.Html_Begin(Map, 1, GEN_BOOKMARKS);
	...
	*/
	...
	if( cookies_found )
	{
		...
		Return_Status = Base_Ptr->GenBookMarks_The_DB(Map, "GenBookMarks.exe", atoi(index.c_str()), 10);
		...
		else if(Return_Status == true)
		{
			...
			// All the output is generated in GenBookMarks_The_DB
		}
		else
		{
	...
	{
		cfl.Html_Begin(Map, 0, GEN_BOOKMARKS, "0");
