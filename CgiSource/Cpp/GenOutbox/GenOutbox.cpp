...
//	cout << HTTPHTMLHeader() << endl << flush;
	...
	string Program_Name("GenOutbox.exe");
	string index = "";
...
//		cout << HTTPHTMLHeader() << endl << flush;
	...
	DB_Table_Base * Base_Ptr = new Login_Info("genOutbox");
	...
	*/
	...
	if( submit != cgi.getElements().end() )
	{
		if (! submit->isEmpty() )
		{
			...
			else if(Return_Status == true)
			{
				...
				//cfl.Html_AddRedirect(Map.PrintValue( "ROOT" ), "GenOutbox.exe");	
			}
			else
			{
				dout << "<!-- ERROR: DB Error. -->\n" << endl;
	...
	{
		cfl.Html_Begin(Map, 0, GEN_OUTBOX, "0");
