...
my $ProgramName 	= "Bible_NewsGroupPoster.pl";
...
my $UniqueID = "Bible_NewsGroupPoster_"  . ($mon+1)  . "_" . $mday  . "_" . ($year+1900) . "_" . $hour  . "_" . $min . "_" . $sec;
...
#		$news_password = "onetwo3";
		...
		$news_server   = "10.10.10.5";
		$news_user 	   = "baumbach";
		$news_password = "super8";
}
else
	{
...
###############################################
######### Get All Bible & Christian Grops #######
###############################################
my @news_groups;
my $group_name_count = 0;
# Get news group names based on whild_card...
(my @list_array) = $nntp->list("active", "*.bible*");
...
# Get news group names based on whild_card...
(my @list_array_2) = $nntp->list("active", "*.christian**");
...
foreach my $list_array_2 (@list_array_2) 
	{
		...
		# make sure the group can be posted to...
		if( ($list_post eq 'y' or $list_post eq 'Y') and $list_group !~ m/bible/)
			{
				# Create the comma seperated news group list for the Newsgroups: header...
				push(@news_groups, "$list_group");
				$group_name_count++;
...
###############################################
####### END Get All Bible & Christian Grops #####
###############################################
		...
		foreach my $news_groups (@news_groups) 
			{
				...
				$highlighted_news_groups =~ s!\.bible!<FONT color="red">\.bible</FONT>!m;
				$highlighted_news_groups =~ s!\.christian!<FONT color="red">\.christian</FONT>!m;
...
while (@news_groups) 
	{
		...
		my $URL = "$Map{'CGIBIN'}/SimpleSearch.exe?index=1&picture_only=0&i_am=1&seeking=2&city=&state=0&country=0&min_age_desired=18&max_age_desired=99&search_limit=5&sort_order=creation_date";
		...
		# SimpleSearch.exe URL for males seeking females...
		my $URL_2 = "$Map{'CGIBIN'}/SimpleSearch.exe?index=1&picture_only=0&i_am=2&seeking=1&city=&state=0&country=0&min_age_desired=18&max_age_desired=99&search_limit=5&sort_order=creation_date";
