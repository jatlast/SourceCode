...
my $ProgramName 	= "Israeli_NewsGroupPoster.pl";
...
my $UniqueID = "Israeli_NewsGroupPoster_"  . ($mon+1)  . "_" . $mday  . "_" . ($year+1900) . "_" . $hour  . "_" . $min . "_" . $sec;
...
# Get news group names based on whild_card...
(my @list_array_2) = $nntp->list("active", "*jewish*");
		...
		foreach my $news_groups (@news_groups) 
			{
				...
				$highlighted_news_groups =~ s!israel!<FONT color="red">israel</FONT>!m;
				$highlighted_news_groups =~ s!jewish!<FONT color="red">jewish</FONT>!m;
