...
my $ProgramName 	= "Manhattan_NewsGroupPoster.pl";
...
my $UniqueID = "Manhattan_NewsGroupPoster_"  . ($mon+1)  . "_" . $mday  . "_" . ($year+1900) . "_" . $hour  . "_" . $min . "_" . $sec;
...
foreach my $list_array (@list_array) 
	{
		...
		# make sure the group can be posted to...
		if( ($list_post eq 'y' or $list_post eq 'Y') and $list_group ne "nyc.transit" and $list_group !~ m/manhattan/)
			{
				# Create the comma seperated news group list for the Newsgroups: header...
				push(@news_groups, "$list_group");
				$group_name_count++;
		...
		foreach my $news_groups (@news_groups) 
			{
				...
				$highlighted_news_groups =~ s!nyc\.!<FONT color="red">nyc\.</FONT>!m;
				$highlighted_news_groups =~ s!manhattan!<FONT color="red">manhattan</FONT>!m;
