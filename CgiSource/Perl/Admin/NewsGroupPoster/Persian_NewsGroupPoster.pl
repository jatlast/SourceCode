...
my $ProgramName 	= "Persian_NewsGroupPoster.pl";
...
my $UniqueID = "Persian_NewsGroupPoster_"  . ($mon+1)  . "_" . $mday  . "_" . ($year+1900) . "_" . $hour  . "_" . $min . "_" . $sec;
...
# Get news group names based on whild_card...
(my @list_array_2) = $nntp->list("active", "*persian*");
		...
		foreach my $news_groups (@news_groups) 
			{
				...
				$highlighted_news_groups =~ s!\.iran!<FONT color="red">\.iran</FONT>!m;
				$highlighted_news_groups =~ s!persian!<FONT color="red">persian</FONT>!m;
