...
private:
    ...
    // Internal functions
    bool tracing(const string &); // Tells whether to do tracing or not
    bool debugging(const string &); // Tells whether to do debugging or not.
                    // Both of these look at the flag and at the vector.
    static void indent();  // Indents the output line
    void Enter();   // Gets us into a function with proper indents,
    ...
    static void setup_output();    // Does what the name implies
    static bool IntTiming(const string &);  // Tells whether to do interval timing
